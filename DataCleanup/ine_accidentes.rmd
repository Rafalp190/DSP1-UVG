...
author: "Rafael León, Julio Barahona y Alejandro Vásquez"
date: "July 29, 2018"
...
#### Libro de Codigos
El libro de codigos se encuentra en el siguiente enlace: (https://github.com/Rafalp190/DSP1-UVG/blob/master/DataCleanup/Results/Codebook.pdf)   
...
fl <- fl %>%
  ...
  mutate(causa_acc = mapvalues(causa_acc, from = as.vector(as.data.frame(table(as.factor(causa_acc)))$Var1), 
                               to = c("Atropello", "Caída","Caída","Choque", "Choque", "Colisi?n","Colisión",
...
fl15 <- fl15 %>%
  ...
  mutate(causa_acc = mapvalues(causa_acc, from = c(1, 2, 3, 4, 5), to = c("Colisi?n", "Choque", "Vuelco", "Caída", "Atropello")))
...
=====================================
Al final se procedió a terminar de limpiar las bases de datos, unificar los tipos de datos y a limpiar outliers, previo a hacer un **full_join**. Además, fue necesario realizar un diccionario de códigos de municipios para convertir algunos municipios de la base de datos "Vehículos involucrados" que se describían por el nombre del municipio, mezclados con códigos.
...
Limpieza de columnas innecesarias, codificación adecuada de variables y conversión de tipos de datos:
```{r}
...
fl_clear <- fl %>%
  ...
  mutate(depto_ocu = mapvalues(depto_ocu, from = as.vector(unique(depto_ocu)), 
                               to = c("Guatemala", "El Progreso", "Sacatepéquez", "Chimaltenango", "Escuintla", "Santa Rosa", "Sololá", "Totonicapán",
                                      ...
                                      "Sacatepéquez", "Sololá", "Totonicapán", "Petén", "Suchitepéquez",  "Santa Rosa", 
                                      "El Progreso"
                                      ))) %>%
  mutate(zona_ocu = ifelse(zona_ocu != 99, zona_ocu, NA)) %>%
  mutate(sexo_pil = ifelse(sexo_pil != 9, sexo_pil, NA)) %>%
  ...
  mutate(estado_pil = as.numeric(estado_pil))
  
```

Por último, realizamos los **joins** finales, una conversión de una variable en el último momento y la escritura del archivo **.csv**:
```{r}
